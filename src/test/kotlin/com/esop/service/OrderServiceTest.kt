...
class OrderServiceTest {
    ...
    fun `It should place BUY order`() {
        //Arrange
        val buyOrder = Order(10, "BUY", 10, userRecords.getUser("sankar")!!)
    ...
    fun `It should place SELL order`() {
        //Arrange
        val sellOrder = Order(10, "SELL", 10, userRecords.getUser("kajal")!!)
    ...
    fun `It should place 2 SELL orders followed by a BUY order where the BUY order is partial`() {
        //Arrange
        userRecords.getUser("kajal")!!.userNonPerfInventory.addESOPsToInventory(50)
        val sellOrderByKajal = Order(10, "SELL", 10, userRecords.getUser("kajal")!!)
        userRecords.getUser("kajal")!!.userNonPerfInventory.moveESOPsFromFreeToLockedState(10)
        orderService.placeOrder(sellOrderByKajal)

        userRecords.getUser("arun")!!.userNonPerfInventory.addESOPsToInventory(50)
        val sellOrderByArun = Order(10, "SELL", 10, userRecords.getUser("arun")!!)
        ...
        val buyOrderBySankar = Order(25, "BUY", 10, userRecords.getUser("sankar")!!)
        ...
        assertEquals("PARTIAL", buyOrders[buyOrders.indexOf(buyOrderBySankar)].getOrderStatus())
        assertEquals(
            "COMPLETED",
            userRecords.getUser("kajal")!!.orderList[userRecords.getUser("kajal")!!.orderList.indexOf(sellOrderByKajal)].getOrderStatus()
        )
        assertEquals(
            "COMPLETED",
            userRecords.getUser("arun")!!.orderList[userRecords.getUser("arun")!!.orderList.indexOf(sellOrderByArun)].getOrderStatus()
        )
    }
    ...
    fun `It should place 2 SELL orders followed by a BUY order where the BUY order is complete`() {
        ...
        userRecords.getUser("sankar")!!.userWallet.addMoneyToWallet(250)
        val buyOrderBySankar = Order(20, "BUY", 10, userRecords.getUser("sankar")!!)
        ...
        assertEquals(
            "COMPLETED",
            userRecords.getUser("sankar")!!.orderList[userRecords.getUser("sankar")!!.orderList.indexOf(buyOrderBySankar)].getOrderStatus()
        )
        assertEquals(
            "COMPLETED",
            userRecords.getUser("kajal")!!.orderList[userRecords.getUser("kajal")!!.orderList.indexOf(sellOrderByKajal)].getOrderStatus()
    ...
    fun `It should place 1 SELL orders followed by a BUY order where the BUY order is complete`() {
        ...
        userRecords.getUser("sankar")!!.userWallet.addMoneyToWallet(250)
        val buyOrderBySankar = Order(5, "BUY", 10, userRecords.getUser("sankar")!!)
    ...
    fun `It should place 1 SELL orders followed by a BUY order where the BUY order is partial`() {
        ...
        userRecords.getUser("sankar")!!.userWallet.addMoneyToWallet(250)
        val buyOrderBySankar = Order(15, "BUY", 10, userRecords.getUser("sankar")!!)
    ...
    fun `It should place 2 BUY orders followed by a SELL order where the SELL order is partial`() {
        //Arrange
        userRecords.getUser("sankar")!!.userWallet.addMoneyToWallet(100)
        val buyOrderBySankar = Order(10, "BUY", 10, userRecords.getUser("sankar")!!)
        userRecords.getUser("sankar")!!.userWallet.moveMoneyFromFreeToLockedState(100)
        orderService.placeOrder(buyOrderBySankar)


        userRecords.getUser("aditya")!!.userWallet.addMoneyToWallet(100)
        val buyOrderByAditya = Order(10, "BUY", 10, userRecords.getUser("aditya")!!)
        userRecords.getUser("sankar")!!.userWallet.moveMoneyFromFreeToLockedState(100)
        ...
        assertEquals(0, userRecords.getUser("sankar")!!.userWallet.getFreeMoney())
        assertEquals(0, userRecords.getUser("sankar")!!.userWallet.getFreeMoney())
        assertEquals("PARTIAL", sellOrders[sellOrders.indexOf(sellOrderByKajal)].getOrderStatus())
        assertEquals(
            "COMPLETED",
            userRecords.getUser("sankar")!!.orderList[userRecords.getUser("sankar")!!.orderList.indexOf(buyOrderBySankar)].getOrderStatus()
        )
        assertEquals(
            "COMPLETED",
            userRecords.getUser("aditya")!!.orderList[userRecords.getUser("aditya")!!.orderList.indexOf(buyOrderByAditya)].getOrderStatus()
        )
    }
    ...
    fun `It should place 2 BUY orders followed by a SELL order where the SELL order is complete`() {
        ...
        val buyOrderByKajal = Order(10, "BUY", 10,  userRecords.getUser("kajal")!!)
        ...
        val buyOrderByArun = Order(10, "BUY", 10,  userRecords.getUser("arun")!!)
        ...
        val sellOrderBySankar = Order(20, "SELL", 10,   userRecords.getUser("sankar")!!)
        ...
        assertEquals(
            "COMPLETED",
            userRecords.getUser("sankar")!!.orderList[userRecords.getUser("sankar")!!.orderList.indexOf(sellOrderBySankar)].getOrderStatus()
        )
        assertEquals(
            "COMPLETED",
            userRecords.getUser("kajal")!!.orderList[userRecords.getUser("kajal")!!.orderList.indexOf(buyOrderByKajal)].getOrderStatus()
        )
        assertEquals(
            "COMPLETED",
            userRecords.getUser("arun")!!.orderList[userRecords.getUser("arun")!!.orderList.indexOf(buyOrderByArun)].getOrderStatus()
        )
    }
    ...
    fun `It should match BUY order for existing SELL order for PERFORMANCE esop type`() {
        ...
        val sellOrder = Order(10, "SELL", 10,  userRecords.getUser("kajal")!!, esopType = "PERFORMANCE")
    ...
    fun `It should match SELL order for PERFORMANCE for existing BUY order`() {
        ...
        userRecords.getUser("kajal")!!.userPerformanceInventory.addESOPsToInventory(50)
        val sellOrder = Order(10, "SELL", 10, userRecords.getUser("kajal")!!, "PERFORMANCE")
        userRecords.getUser("kajal")!!.userPerformanceInventory.moveESOPsFromFreeToLockedState(10)
    ...
    fun `It should match SELL order for existing BUY order where SELL order is complete`() {
        ...
        val buyOrder = Order(20, "BUY", 10, userRecords.getUser("sankar")!!)
        userRecords.getUser("sankar")!!.userWallet.moveMoneyFromFreeToLockedState(200)
        orderService.placeOrder(buyOrder)
    ...
    fun `It should place 2 SELL orders where one SELL order is of PERFORMANCE esop type and other is of NON-PERFORMANCE esop type followed by a BUY order where the BUY order is complete`() {
        ...
        val sellOrderByArun = Order(10, "SELL", 10, userRecords.getUser("arun")!!, "PERFORMANCE")
    ...
    fun `It should place 2 SELL orders of PERFORMANCE esop type followed by a BUY order where the BUY order is complete`() {
        //Arrange
        userRecords.getUser("kajal")!!.userPerformanceInventory.addESOPsToInventory(50)
        val sellOrderByKajal = Order(10, "SELL", 10, userRecords.getUser("kajal")!!, "PERFORMANCE")
        userRecords.getUser("kajal")!!.userPerformanceInventory.moveESOPsFromFreeToLockedState(10)
        orderService.placeOrder(sellOrderByKajal)
    ...
    fun `It should place 2 SELL orders of NON-PERFORMANCE esop type followed by a BUY order where SELL order price is different`() {
        //Arrange
        userRecords.getUser("kajal")!!.userNonPerfInventory.addESOPsToInventory(50)
        val sellOrderByKajal = Order(10, "SELL", 20, userRecords.getUser("kajal")!!)
        userRecords.getUser("kajal")!!.userNonPerfInventory.moveESOPsFromFreeToLockedState(10)
        orderService.placeOrder(sellOrderByKajal)
        ...
        val buyOrderBySankar = Order(20, "BUY", 20, userRecords.getUser("sankar")!!)
